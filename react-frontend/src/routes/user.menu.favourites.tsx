...
import { getFavouriteDishes, getDislikedDishes, getLikedDishes } from '@/lib/dishPreferences'

export const Route = createFileRoute('/user/menu/favourites')({
...
function RouteComponent() {
  ...
  const handlePlaasBestelling = () => {
    const bestelling = filteredMeals?.filter(meal => (menuQuantity[meal?.id ?? -1] ?? -1) > 0) as any[] ?? []
  ...
  const favIds = getFavouriteDishes()
  ...
  let filteredMeals = (meals ?? []).filter(meal => favIds.includes(Number(meal.id)))
  ...
  const totalCost = filteredMeals?.reduce((sum, meal) => {
