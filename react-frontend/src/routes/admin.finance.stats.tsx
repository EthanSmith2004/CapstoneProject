import { AdminFinanceApi } from '@/api';
...
import { AdminFinanceStats } from '@/components/admin/AdminFinanceStats';
...
function RouteComponent() {
  ...
  const {data: financialSummary, isLoading: financialSummaryLoading} = useQuery({
    queryKey: ['financialSummary', statisticPeriod.start, statisticPeriod.end],
    queryFn: async () => (await financeAPI.getStatistics(statisticPeriod.start.toISOString(), statisticPeriod.end.toISOString())).data,
    staleTime: 1000 * 60 * 5, // 5 minutes
  });

  const {data: previousFinancialSummary} = useQuery({
    queryKey: ['financialSummary', statisticPeriod.previousStart, statisticPeriod.previousEnd],
    queryFn: async () => {
      if (statisticPeriod.previousStart && statisticPeriod.previousEnd) {
        return (await financeAPI.getStatistics(statisticPeriod.previousStart.toISOString(), statisticPeriod.previousEnd.toISOString())).data;
      }
      return null;
    },
  ...
  return (
    <div className="p-4 flex flex-col lg:flex-row">
      ...
      <div className='ml-8 flex-1'>
        {!financialSummaryLoading && <AdminFinanceStats 
          stat={financialSummary!} 
          previousStat={previousFinancialSummary}
          contrastName={statisticPeriod.contrastName}
        />}
        {financialSummaryLoading && <AdminLoader />}
      </div>
    </div>
  );
}
